...
def resolve_names(file_list, old_map_name, new_map_name):
	...
	re_cubemap = re.compile(r".+-?\d+_-?\d+_-?\d+$")
		...
		if extension == ".txt":
			...
			if dirname.startswith(map_manifest_path_prefix):
				if item.basename_matches_suffix(old_map_name, "_level_sounds"):
					...
					item.merge_function = merge_level_sounds_txt_data
					...
					item.merge_function = merge_particles_txt_data
			# particles/
		...
		with zipfile.ZipFile(pakdata_in, mode="r") as pakzip_in:
			for item in file_list:
				if item.can_merge() and item.is_multi_source():
					# Will be an empty byte array if not found.
					existing_data = pak.get_file_data(pakzip_in, item.in_pak_path)

					new_data = bytes()

					if os.path.isfile(item.in_disk_path):
						with open(item.in_disk_path, "rb") as infile:
							new_data = infile.read()

					print(f"Merging {item.out_pak_path}:")
					print(f"  + {item.in_pak_path}")
					print(f"  + {item.in_disk_path}")
					item.merge_function(existing_data, new_data)
				else:
					if item.in_disk_path:
						print(f"Embedding {item.out_pak_path}")
						pak.try_write_disk_file(pakzip_out, item.in_disk_path, item.out_pak_path)
					else:
						...
						pak.try_write_data(pakzip_out, item.out_pak_path, pak.get_file_data(pakzip_in, item.in_pak_path))
